...
- Docker — **required**, not optional: `pre-commit` regenerates `docs/workflows/*.md`
  via a Dockerized `actdocs`, in addition to testing workflows locally with `act`
...
10. **generate-workflow-docs** - Regenerates `docs/workflows/*.md` from each reusable workflow's `on.workflow_call` block

### Regenerating the workflow reference docs

`docs/workflows/*.md` — one file per reusable (`workflow_call`) workflow, linked from
each workflow's section in [README.md](../README.md) — is generated by
[`actdocs`](https://github.com/tmknom/actdocs) reading the workflow's own `inputs`,
`secrets`, and `outputs` declarations. **Never edit a file under `docs/workflows/` by
hand** — the pre-commit hook overwrites it and, because it wipes and rebuilds the whole
directory every run, also deletes it outright once the workflow it names is gone.

To document a new reusable workflow, or pick up a changed input/secret/output, just
run the hook (`pre-commit run generate-workflow-docs --all-files`, or
`.pre-commit-scripts/generate-workflow-docs.sh` directly) — nothing to wire up by hand.
The generated tables are only as good as the source workflow's own `description`
fields, though: an `inputs`/`secrets`/`outputs` entry with no `description` renders
with an empty cell, so write real descriptions in the workflow file itself rather than
patching them into the generated doc afterward.
